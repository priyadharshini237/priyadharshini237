# Hi There 👋 I'm Priyadharshini

>🎓 Master's Graduate in Bioinformatics
>📊 Aspiring Data Analyst | Transitioning into IT

### 🚀 About me
- 🎓 Completed Master’s in Bioinformatics
- 🧪 Worked on Drug discovery data analysis in my final year project
- 💼 5 months experience in Editorial Support at Straive
- 🔄 Currently transitioning into the Data Analyst field
- 📈 Passionate about turning data into meaningful insights
### 📫 Connect With Me Here

<p>
  <a href="www.linkedin.com/in/priyadharshini2307" target="_blank">
    <img width="60" height="60" src="https://img.icons8.com/3d-fluency/60/linkedin--v1.png" alt="linkedin--v1"/>
  </a>

  <a href="mpriyad23@gmail.com">
    <img width="60" height="60" src="https://img.icons8.com/nolan/60/gmail-new.png" alt="gmail-new"/>
  </a>
</p>

---

### 🛠️ Skills & Tools
##### 💻 *Programming & Data Analysis*
-🐍 Python
-🐼 Pandas
-🔢 Numpy

##### 📊 *Data Visualization*
-📈 Power BI
-📉 Tableau
-📊 Matplotlib

##### 🗄*Databases & Tools*
-💻 SQL
-📑 Excel

---

### 🛠️ Tools i used
<p>
<img width="60" height="60" src="https://img.icons8.com/arcade/60/python.png" alt="python"/>
<img width="60" height="60" src="https://img.icons8.com/arcade/60/sql.png" alt="sql"/>
<img width="60" height="60" src="https://img.icons8.com/color/60/pandas.png" alt="pandas"/>
<img width="60" height="60" src="https://img.icons8.com/fluency/60/power-bi-2021.png" alt="power-bi-2021"/>
<img width="60" height="60" src="https://img.icons8.com/color/60/tableau-software.png" alt="tableau-software"/>
<img width="60" height="60" src="https://img.icons8.com/doodle/60/microsoft-excel-2019.png" alt="microsoft-excel-2019"/>
<img src="https://resources.jetbrains.com/storage/products/company/brand/logos/PyCharm_icon.png" alt="PyCharm" width="60" height="60"/>
<img width="60" height="60" src="https://img.icons8.com/color/60/matplotlib.png" alt="matplotlib"/>
<img width="60" height="60" src="https://img.icons8.com/color/60/numpy.png" alt="numpy"/>
</p> 

---

### 💼 Experience
#### Editorial Support Associate — Straive
- 🔍 Performed quality checks on research articles to ensure accuracy, clarity, and consistency
- ✅ Ensured error-free content by identifying and correcting inconsistencies
- 🤝 Collaborated with teams to ensure smooth editorial workflow
- 📊 Developed strong analytical and data handling skills through content validation

### 🎯 Career Goal
💡 To become a Data Analyst in the IT industry and solve real-world problems using data







